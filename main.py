...
class ONVIFDetailWindow:
    def __init__(self, parent, camera_index, cameras, models, app_instance, data_dir):
        ...
        self.use_web_creds = tk.BooleanVar(value=False)
        self.window = tk.Toplevel(parent)
        self.window.title(f"ONVIF Детали камеры: {self.camera.get('model', 'Неизвестно')} ({self.camera.get('ip', 'Нет IP')})")
        self.window.geometry("900x800")
        center_window(self.window, parent)
        self.window.transient(parent)
        self.window.grab_set()
        ...
        if self.use_web_creds.get():
            # ✅ ИСПРАВЛЕНО: используем quote с safe-символами, чтобы $ и @ не кодировались
            user = urllib.parse.quote(self.camera.get("web_user", ""), safe='@$')
            password = urllib.parse.quote(self.camera.get("web_pass", ""), safe='@$')
        else:
            # ✅ ИСПРАВЛЕНО: используем quote с safe-символами, чтобы $ и @ не кодировались
            user = urllib.parse.quote(self.camera.get("stream_user", ""), safe='@$')
            password = urllib.parse.quote(self.camera.get("stream_pass", ""), safe='@$')
        self.onvif_url = f"http://{user}:{password}@{ip}/onvif/device_service" if ip else "Не удалось сформировать ONVIF URL"

        self.use_web_check = tk.Checkbutton(scrollable_frame, text="Использовать Web логин/пароль", variable=self.use_web_creds, font=LARGE_FONT, command=self.update_url)
        self.use_web_check.grid(row=row, column=0, columnspan=2, sticky="w", padx=10, pady=5)
        row += 1

        tk.Label(scrollable_frame, text="ONVIF Endpoint:", font=BOLD_FONT, anchor="w").grid(row=row, column=0, sticky="w", pady=5, padx=10)
        self.url_label = tk.Label(scrollable_frame, text=self.onvif_url, font=("Courier", 10), fg="blue", anchor="w", wraplength=500, justify="left")
        self.url_label.grid(row=row, column=1, sticky="w", padx=10, pady=5)
        row += 1
        ...
        tk.Button(btn_frame_top, text="💾 Сохранить изменения", command=self.save_changes, font=LARGE_FONT, bg="#a0f0a0").pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame_top, text="📸 Сделать скриншот", command=self.load_screenshot, font=LARGE_FONT, bg="#a0d0f0").pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame_top, text="📋 Копировать ONVIF URL", command=self.copy_onvif_url, font=LARGE_FONT).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame_top, text="❌ Закрыть", command=self.window.destroy, font=LARGE_FONT).pack(side=tk.LEFT, padx=5)
        row += 1
    ...
    def copy_onvif_url(self):
        if self.onvif_url and "Не удалось" not in self.onvif_url:
            self.window.clipboard_clear()
            self.window.clipboard_append(self.onvif_url)
            self.window.update()
            messagebox.showinfo("Копирование", "ONVIF URL скопирован в буфер обмена!")
        else:
            messagebox.showwarning("Ошибка", "Невозможно скопировать: ONVIF URL не сформирован.")
...
class CameraApp:
    ...
    def open_onvif_detail(self, item_id):
        ...
        for i, cam in enumerate(self.cameras):
            if (cam.get("line") == filtered_cam.get("line") and
                ...
                break
        if real_idx is not None:
            ONVIFDetailWindow(self.root, real_idx, self.cameras, self.models, self, self.data_dir)
    ...
    def export_pdf(self):
        ...
        if filepath:
            # ✅ Передаем каталог данных в функцию экспорта
    ...
    def show_info(self):
        messagebox.showinfo("Информация о программе", "Программа \"Datacam\" версия 0.1\nАвтор: Разин Г.В.\n© 2025")
